...
class JwtService(
    ...
    fun createJwtToken(loginRequest: LoginRequest): String? {
        ...
        return if (foundUser != null && loginRequest.password == foundUser.password) JWT.create().withAudience(audience)
            .withIssuer(issuer).withClaim("username", loginRequest.username)
            .withExpiresAt(Date(System.currentTimeMillis() + 3_600_000)).sign(Algorithm.HMAC256(secret))
        else null
    }
